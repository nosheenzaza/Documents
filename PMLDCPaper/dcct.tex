...
in[cloud types],
which proposes a similar system. {@nate actually going back to that, I forgot 
...
Catogories are data-centric approaches, tools and theories to reason about
multiple consistency policies, and classifying consistency required to satisfy
invariants, most works mentioned here fall under more than one category.
...
is similar to our notion of actions in the sense they do not enforce a single
isolation property, however, they assume that the
they also allow some data regions to have adaptable consistency, and
accept invariant violations at a monetary cost. While they aim to reduce
monetary costs resulting from loss of consistency or delay caused by over
synchronization, we aim to maintain the consistency level required to enforce
certain invariants at all times.

% bailis i confluence analysis 
Bailis el al{feral} describe
a.... In his thesis work and a number of papers{refs}, 

% putting consistency back, i-offender analysis
extension of i confluence to include invariants that require coordination as
well. Invariants attached to data. Because invariants need to be mapped to
consistency properties, not all invariants found in applications can be
expressed. 
...
% Indigo: Cause I am strong enough, a proof rule relying on invariant declarations 
Gotsman et al[] propose a proof rule to show that a particular choice of consistency guarantees for operations
ensures the preservation of programmer-defined invariants. The system, similar
to Karska's also guarantees at least causal consistency, the diference is that
it focuses on effecient invariant inforcement rather than the most effective
monetary cost. It also requires the programmer to write proof rules.

% Quelea language
Quelea also uses simple first order logic predicates to describe operations sequencing and
...
The main advantage of our system is simplicity and accomodating any consistency
policy, as long as its strength is comparable to other consistency leves in the
system. On the down side, it requires whole program analysis, and since we
consider only low level write operations, we cannot allow semantics such as
those in the bank example of quelea, where higher level operations can execute
